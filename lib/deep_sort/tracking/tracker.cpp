...
#include "tracking/nn_matching.h"
#include "dataType.h"
...
void tracker::update(const DETECTIONS &detections)
{
    ...
    for(Track track:tracks) {
        ...
        track.features = Eigen::Matrix<float, -1, FEATURE_NUM, Eigen::RowMajor>(0,FEATURE_NUM);
    }
    ...
    */
    ...
    for (Track& track:tracks) {
        ...
        FEATURESS t = FEATURESS(0, FEATURE_NUM);
        track.features = t;
    }
...
{
    FEATURESS features(detection_indices.size(), FEATURE_NUM);
    int pos = 0;
